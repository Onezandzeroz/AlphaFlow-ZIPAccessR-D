...
export async function GET(request: NextRequest) {
  try {
    ...
    // Customers — derive from contacts used in journal entries / invoices
    const customers = masterFiles.ele('Customers');

    // Collect unique contact IDs from invoices in the period
    ...
    // Also collect contact IDs from contacts referenced in journal entries
    const distinctContactIds = new Set(periodInvoices.map(inv => inv.contactId));

    // Fetch all contacts for the tenant (used in period or all for completeness)
    ...
    if (contactsToExport.length > 0) {
      contactsToExport.forEach(contact => {
        ...
        // Customer address (from contact fields)
      ...
      // Fallback: no contacts in system — emit a minimal placeholder
      const customerNode = customers.ele('Customer');
